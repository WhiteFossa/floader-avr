...
/* Версия 0.0.2 (порт) от 1.09.2012                                              */
/* Порт FTX-2.0 (передатчики для охоты на лис Фетюлина Евгения)                  */
...
.equ		LISTEN_BITS		=		0b00000111				//Битовая маска условия входа
//Вход осуществляется в том случае, если биты, равные 1 в маске равны 0 на LISTEN_PORT

.equ		LED_PORT		=		PORTC					//На каком порту зажигать светодиод
.equ		LED_DDR			=		DDRC					//Порт направления
.equ		LED_BIT0		=		0						//Первый светодиод
.equ		LED_BIT1		=		3						//Второй светодиод

.equ		OSC_FREQ		=		2000000					//Частота кварца
...
.equ		UBRR0H_MASK		=		0xF0					//Маска для UBRR0H
...
//МК-специфические дефайны (ATmega168)
...
//Замедляем сторожевой таймер до предела и пытаемся выключить
cli															//Подавляем прерывания
wdr															//Сбрасываем на всякий случай

//Сбрасываем флаг сброса от сторожевого таймера (если стоял)
in			R16,			MCUSR
andi		R16,			(0xFF & (0 << WDRF))
out			MCUSR,			R16

ldi			R17,			(1<<WDP3)|(1<<WDP0)	//Интервал - 8 секунд, таймер - отключен

//Взводим WDTCE и WDE с сохранением предделителя
lds			R16,			WDTCSR
ori			R16,			(1<<WDCE)|(1<<WDE)
sts			WDTCSR,			R16

//Задаём новый интервал и отключаем таймер
sts			WDTCSR,			R17

//ВНИМАНИЕ! ЕСЛИ СТОИТ ФУЗА WDTON, ТО WDT НЕЛЬЗЯ ОТКЛЮЧИТЬ ПРОГРАММНЫМ ПУТЁМ. ПОСЕМУ БУДЕМ
//ВЕСТИ СЕБЯ ТАК, КАК БУДТО ОН ВКЛЮЧЁН
...
clr			R16
out			LISTEN_DDR,		R16								//Все ноги LISTEN_DDR на ввод

ldi			R16,			0xFF							//Включаем подтягивающие резисторы
out			LISTEN_PORT,	R16
...
and			R16,			R17
breq		ENTER
jmp			RESET											//Не все ноги нажаты, уходим на основную программу
...
//Зажигаем светодиод
sbi			LED_DDR,		LED_BIT0
sbi			LED_DDR,		LED_BIT1
sbi			LED_PORT,		LED_BIT0
sbi			LED_PORT,		LED_BIT1

//Настраиваем UART
ldi			R16,			low(BAUDDIVIDER)
sts			UBRR0L,			R16
ldi			R16,			high(BAUDDIVIDER)
andi		R16,			UBRR0H_MASK
sts			UBRR0H,			R16
clr			R16
sts			UCSR0A,			R16
ldi 		R16, 			(1<<RXEN0)|(1<<TXEN0) //Включаем приём и передачу
sts			UCSR0B,			R16
ldi 		R16, 			(1<<UCSZ00)|(1<<UCSZ01) //Один стоповый бит, 8 бит данных, без проверки чётности
sts			UCSR0C,			R16
...
//Ждём завершения передачи (TXC0, не UDRE0)
TX_NOT_COMPLETED:
wdr
lds			R16,			UCSR0A
sbrs		R16,			TXC0
...
//Гасим светодиоды и возвращаем порты в исходное состояние
cbi			LED_PORT,		LED_BIT0
cbi			LED_PORT,		LED_BIT1
cbi			LED_DDR,		LED_BIT0
cbi			LED_DDR,		LED_BIT1
